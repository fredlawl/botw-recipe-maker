...
import {primaryCategories, Category} from "../database/itemCategories";

describe("Inventory.addInventoryItem()", () => {
	test("add inventory item works with 1 item", () => {
		const expectedStack = 20;
		const inventory = new Inventory();
		const strawberry = new Material("Strawberry", [ primaryCategories[Category.FRUIT] ], [], ImmunityBuffType.NONE);
	...
	test("add inventory works with more than 1 item", () => {
		const inventory = new Inventory();
		const strawberry = new Material("Strawberry", [ primaryCategories[Category.FRUIT] ], [], ImmunityBuffType.NONE);
		const banana = new Material("Banana", [ primaryCategories[Category.FRUIT] ], [], ImmunityBuffType.NONE);
